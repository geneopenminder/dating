...
public class FNameFilter extends BaseFilter {
    ...
    public static int notNullFNamesCount = 0;
    ...
    public void init() {
        ...
        for (int i = 1; i < ArrayDataAllocator.ACCOUNTS_TOTAL.get() + 1; i++) {
            ...
            if (nameCode != Short.MAX_VALUE) {
                ...
                long[] fnameAccsMap = fNameAccountsBitMaps.get(nameCode);
                if (fnameAccsMap == null) {
                    fnameAccsMap = DirectMemoryAllocator.alloc(MAX_ACCOUNTS);
                    fNameAccountsBitMaps.put(nameCode, fnameAccsMap);
                }
                DirectMemoryAllocator.setBit(fnameAccsMap, i);
            } else {
                nullFNamesCount++;
...
*/
    ...
    public void processNew(InternalAccount a) {
        ...
        if (nameCode != Short.MAX_VALUE) {
            ...
            DirectMemoryAllocator.setBit(fnameAccsMap, a.id);
        } else {
            nullFNamesCount++;
    ...
    public void processUpdate(InternalAccount account) {
        short oldIdx = fnamesGlobal[account.id];

        short newNameIdx = account.fname;

        if (oldIdx == Short.MAX_VALUE) {
            notNullFNamesCount++;
            DirectMemoryAllocator.setBit(notNullFNamesBitMap, account.id);
            DirectMemoryAllocator.unsetBit(nullFNamesBitMap, account.id);
            countByFName.merge(newNameIdx, 1, Integer::sum);
        } else {
            countByFName.merge(oldIdx, -1, Integer::sum);
            countByFName.merge(newNameIdx, 1, Integer::sum);

            //remove from old name index
            long[] fnameAccsMap = fNameAccountsBitMaps.get(oldIdx);
            DirectMemoryAllocator.unsetBit(fnameAccsMap, account.id);

        }

        long[] fnameAccsMap = fNameAccountsBitMaps.get(newNameIdx);
        if (fnameAccsMap == null) {
            fnameAccsMap = DirectMemoryAllocator.alloc(MAX_ACCOUNTS);
            fNameAccountsBitMaps.put(newNameIdx, fnameAccsMap);
        }
        DirectMemoryAllocator.setBit(fnameAccsMap, account.id);
    ...
    public void calculatePriority() {
        if (predicate.equalsIgnoreCase("fname_eq") || predicate.equalsIgnoreCase("fname")) {
            ...
            if (value.equalsIgnoreCase("0") ) {
                calculatedPriority = notNullFNamesCount;
